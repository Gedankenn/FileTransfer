...
static size_t write_callback(void *contents, size_t size, size_t nmemb, void *userp)
{
    ...
    if (max_append > 0) {
        strncat((char *)userp, (char *)contents, max_append < total_size ? max_append : total_size);
    }
...
int find_my_ip(int type, char* ip)
{
    ...
    if (curl) {
        ...
        curl_easy_setopt(curl, CURLOPT_USERAGENT, "curl/7.81.0");  // Required by ifconfig.me
